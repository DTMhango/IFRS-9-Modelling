...
def clean_dataframe(my_df: pd.DataFrame) -> tuple:
    ...
    my_df["current_date"] = pd.to_datetime(my_df["date"], errors="coerce", dayfirst=True)

    # convert dates if format type is excel day counts
    mask = pd.to_numeric(my_df["date"], errors="coerce").notna()
    my_df.loc[mask, "current_date"] = pd.to_datetime(my_df["date"][mask].astype(float), errors="coerce", unit="D", origin="1899-12-30")

    my_df["out_balance"] = (my_df["out_balance"].astype(str).str.replace("(", "-").str.replace(r"[^0-9.-]", "", regex=True))
    my_df["days_past_due"] = (my_df["days_past_due"].astype(str).str.replace("(", "-").str.replace(r"[^0-9.-]", "", regex=True))

    my_df["out_balance"] = pd.to_numeric(my_df["out_balance"], errors="coerce")
    my_df["days_past_due"] = pd.to_numeric(my_df["days_past_due"], errors="coerce")
...
def date_ccleaner(df: pd.DataFrame, old_column: str, new_column: str) -> pd.DataFrame:
...
def data_prep(data: pd.DataFrame, period: int, matrix_size: int = 3) -> tuple:
    ...
    - period: transition granularity -> 1, 3, 6, 12 for monthly, quarterly, semi-annual or annual periods respectively
    ...
    if period not in {1, 3, 6, 12}:
        raise ValueError("Period not recognized. Must be 1, 3, 6 or 12")
    ...
    return pd_data, error_data, duplicate_id


def recoveries_prep(data: pd.DataFrame, period: int) -> tuple:
    ...
    # duplicate_id = recoveries_data[recoveries_data.duplicated(subset='current_id', keep='first')]  # store duplicate values for assessment

    # recoveries_data = recoveries_data.drop_duplicates(subset='current_id', keep='first').reset_index(drop=True)  # drop duplicates to ensure identifier is unique
    ...
    return discounted_cash_recoveries, error_data  # , duplicate_id


def merge_recoveries(pd_df: pd.DataFrame, recoveries_df: pd.DataFrame) -> pd.DataFrame:
    """Merge the historical pd dataframe with the recoveries dataframe and identify recoveries"""
    ...
    conditions = ((final_df["current_stage"] == "stage_3") & (final_df["cures"] != "cured") & (final_df["discounted_recoveries"] > 0))

    final_df["recoveries"] = np.where(conditions, "recovered", final_df["cures"])
